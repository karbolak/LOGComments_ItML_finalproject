...
def train_final_model(X_train, y_train, best_hparams) -> LogisticRegression:
    ...
    plot_learning_curves(train_losses=final_model.train_losses, val_losses=final_model.val_losses)

    return final_model


def evaluate_model(final_model, X_test, y_test) -> np.ndarray:
    ...
    for name, calculator in metrics_calculators.items():
        if name == 'Log Loss':
            score = calculator(y_test, final_model._sigmoid(np.dot(X_test, final_model.weights) + final_model.bias))
        else:
